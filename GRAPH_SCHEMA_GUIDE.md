...
Spanner には全部で **22 テーブル**（ノード6 + エッジ16）と、それらを束ねる **1 つのプロパティグラフ `code_graph_a`** がある。
...
| **CrossRepoRef** | Class → Class | **リポジトリ間**のクラス参照 | `source_repo`, `target_repo`, `symbol`（FQCN）, `kind`（`import` / `class_ref` / `extends` / `implements` / `uses` / `new` / `type_hint` / `instanceof`） | `crossref` コマンドが、全 repo の entities から作った FQCN レジストリで「別 repo に定義されたクラスへの参照」を PHP の名前解決規則どおりに確定したときだけ（クラス名はグローバルへフォールバックしない・vendor ミラーは所有者にならない・参照元 repo 自身の定義が勝つ・複数 repo 定義は曖昧としてレポート） |
| **CrossRepoFileRef** | File → Class | クラスを定義しないファイル（bootstrap・設定・ルーティング等）からの**リポジトリ間**クラス参照 | `source_repo`, `target_repo`, `symbol`（FQCN）, `kind`（CrossRepoRef と同じ） | `crossref` が、クラスを持たないファイルの `use` インポートやトップレベル関数内の参照を確定したときだけ |
| **CrossRepoCalls** | Method → Method | **リポジトリ間**のメソッド呼び出し・名前空間関数呼び出し | `source_repo`, `target_repo`, `symbol`（FQMN） | `crossref` が、レシーバ型が別 repo のクラスに解決できた呼び出し（型付き引数・コンストラクタ注入プロパティ・戻り値型が宣言されたメソッド経由の連鎖レシーバ）と、別 repo 定義の名前空間関数呼び出しを確定したときだけ（型が特定できない動的呼び出しは対象外） |
| **DiBinds** | Class → Class | DI コンテナのインターフェース→具象束縛 | `source_repo`, `target_repo`, `symbol`（具象 FQCN） | `services()` の `add(I::class, C::class)`（`di_parser`）から。同一 repo 内は Phase 9、別 repo 間は `crossref` |
| **DiInjects** | Class → Class | DI コンテナのサービス→依存注入 | `source_repo`, `target_repo`, `symbol`（依存 FQCN） | `services()` の `add(S::class)->addArgument(D::class)` から。同一 repo 内は Phase 9、別 repo 間は `crossref` |

すべてのエッジテーブルは `edge_id`（主キー）＋ source 列 ＋ destination 列（＋補助列）という共通の形をしている。16種すべてにパイプライン／`crossref` がデータを投入する（Modules が無ければ FileBelongsToModule は 0 行、マイグレーションが無ければ DbTables 系は 0 行、単一リポジトリなら CrossRepo* は 0 行）。`CrossRepoRef` / `CrossRepoFileRef` / `CrossRepoCalls` は**別リポジトリ間の依存だけ**を表す（同一リポジトリ内の呼び出し・継承は MethodCalls / ClassInherits など）。`crossref` はエッジにしなかった参照も黙って捨てず、曖昧・未所有の内訳をコンソールと `OUTPUT_DIR/crossref_report.json` にレポートする。詳細は [MULTI_REPO_GUIDE.md](MULTI_REPO_GUIDE.md)。

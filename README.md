...
| `analyze <target_dir>` | **フルパイプライン**（ドキュメント + グラフ + エンベディング）。複数リポジトリは `--repo-name` / `--repos` で 1 グラフに同居 | Phase 1〜10 |
...
| `crossref` | 複数リポジトリ投入後、**リポジトリ間の依存（影響分析）と結合度**を導出してグラフに書き込む（詳細は [MULTI_REPO_GUIDE.md](MULTI_REPO_GUIDE.md)） | -- |
| `validate` | Spanner グラフの行数カウントと孤立エッジ検出（PossiblyCalls / TableReferences / CrossRepo* 等を含む）＋リポジトリ別ノード件数・結合度マトリクスを表示 | -- |
...
- `analyze` / `generate wiki` / `generate graph` は `--repo-name NAME`（複数リポジトリを 1 つのグラフに同居させるための識別名。既定はディレクトリ名）と、複数リポジトリを一括投入する `--repos <manifest.json>` を受け付けます（詳細は [複数リポジトリの取り扱い](#複数リポジトリの取り扱い)）。
...
リポジトリ同士は**独立した島（isolated islands）**として扱われ、リポジトリ A から
リポジトリ B のシンボルへの参照は vendor と同様に `external`（集計のみ・エッジなし）となります
— リポジトリをまたぐ誤ったエッジは構造的に発生しません。

投入方法は 2 通り（どちらも同じグラフに追記されます）:

```bash
# 1) 1 リポジトリずつ（インクリメンタル。後から追加・再投入も可能）
...
`repos.json` の形式（`name` 省略時はディレクトリ名、`include_vendor` は任意）:
...
- リポジトリごとのチェックポイント／中間生成物は `output_docs_pipeline/repos/<repo>/` 配下に
  分離されるため、互いに干渉しません。
- 同名メソッドを FQMN で一意特定でき、`validate` は `repo` ごとのノード件数を表示するので
  投入漏れや同名リポジトリの取り違えにすぐ気付けます。
- 既存グラフ（`repo` 導入前）は ID 体系が変わるため、新しい `GRAPH_NAME`/`ID_PREFIX`
  で作り直すか `setup_spanner_graph --destroy` 後に再構築してください（`--migrate` は `repo`
  カラム追加を冪等に行いますが、旧 ID の行はそのまま残ります）。

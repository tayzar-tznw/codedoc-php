...
**Graph & query stack:** `graph_generator/setup_spanner_graph.py` (declarative `SCHEMA_SPEC`, single source of truth for DDL + the pipeline's insert column lists via `write_columns`) creates 6 node tables (Files/Classes/Methods/Modules/Directories/DbTables) + 11 edge tables (adds PossiblyCalls, TableReferences, ClassMapsToTable; FileImports now populated) + one property graph (`config.GRAPH_NAME`, default `code_graph_a`). Existing DBs migrate idempotently via `python -m graph_generator.setup_spanner_graph --migrate` (INFORMATION_SCHEMA diff → ALTER/CREATE). `graph_query_agent` is a two-ADK-agent setup — `root_agent` (orchestrator, no tools) delegates to `graph_agent`, which holds the read-only `run_gql_query` tool. It is consumed by `mcp_server/` (FastMCP over HTTP) and `webapp/main.py` (FastAPI: serves generated docs from `OUTPUT_DIR` + `/api/chat` endpoints).
